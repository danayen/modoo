...
def flush_env(cr, *, clear=True):
    warnings.warn("Since modoo 15.0, use cr.flush() instead of flush_env(cr).",
...
def clear_env(cr):
    warnings.warn("Since modoo 15.0, use cr.clear() instead of clear_env(cr).",
                  DeprecationWarning, stacklevel=2)
    cr.clear()
...
class Cursor(BaseCursor):
    ...
    def autocommit(self, on):
        if on:
            warnings.warn(
                "Since modoo 13.0, the ORM delays UPDATE queries for "
...
def close_db(db_name):
    """ You might want to call modoo.modules.registry.Registry.delete(db_name) along this function."""
    global _Pool
    if _Pool:
