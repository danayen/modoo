...
class Shell(Command):
    """Start modoo in an interactive shell"""
    supported_shells = ['ipython', 'ptpython', 'bpython', 'python']

    def init(self, args):
        config.parse_config(args)
        modoo.cli.server.report_configuration()
        modoo.service.server.start(preload=[], stop=True)
    ...
    def shell(self, dbname):
        ...
        if dbname:
            ...
            with registry.cursor() as cr:
                ...
                ctx = modoo.api.Environment(cr, uid, {})['res.users'].context_get()
                env = modoo.api.Environment(cr, uid, ctx)
                local_vars['env'] = env
