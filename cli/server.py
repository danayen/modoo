...
__author__ = modoo.release.author
__version__ = modoo.release.version

# Also use the `modoo` logger for the main script.
_logger = logging.getLogger('modoo')
...
def report_configuration():
    ...
    """
    config = modoo.tools.config
    _logger.info("modoo version %s", __version__)
    if os.path.isfile(config.rcfile):
        _logger.info("Using configuration file at " + config.rcfile)
    _logger.info('addons paths: %s', modoo.addons.__path__)
...
def setup_pid_file():
    ...
    """
    config = modoo.tools.config
    if not modoo.evented and config['pidfile']:
        pid = os.getpid()
...
def export_translation():
    ...
    with open(config["translate_out"], "wb") as buf:
        registry = modoo.modules.registry.Registry.new(dbname)
        with registry.cursor() as cr:
            modoo.tools.trans_export(config["language"],
...
def import_translation():
    ...
    registry = modoo.modules.registry.Registry.new(dbname)
    with registry.cursor() as cr:
        modoo.tools.trans_load(
            cr, config["translate_in"], config["language"], overwrite=overwrite,
        )

def main(args):
    check_root_user()
    modoo.tools.config.parse_config(args)
    check_postgres_user()
    ...
    if config['db_name']:
        ...
        for db_name in preload:
            try:
                modoo.service.db._create_empty_database(db_name)
                ...
                if err.pgcode == errorcodes.INSUFFICIENT_PRIVILEGE:
                    ...
                    raise err
            except modoo.service.db.DatabaseExists:
                pass
    ...
    if config['workers']:
        modoo.multi_process = True

    stop = config["stop_after_init"]

    setup_pid_file()
    rc = modoo.service.server.start(preload=preload, stop=stop)
    sys.exit(rc)
