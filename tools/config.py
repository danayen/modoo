...
class configmanager(object):
    def __init__(self, fname=None):
        ...
        group.add_option("-s", "--save", action="store_true", dest="save", default=False,
                          help="save configuration to ~/.modoorc (or to ~/.openerp_serverrc if it exists)")
        ...
        temp_tests_dir = os.path.join(tempfile.gettempdir(), 'modoo_tests')
        ...
        group.add_option('--log-handler', action="append", default=[], my_default=DEFAULT_LOG_HANDLER, metavar="PREFIX:LEVEL", help='setup a handler at LEVEL for a given PREFIX. An empty PREFIX indicates the root logger. This option can be repeated. Example: "modoo.orm:DEBUG" or "werkzeug:CRITICAL" (default: ":INFO")')
        group.add_option('--log-request', action="append_const", dest="log_handler", const="modoo.http.rpc.request:DEBUG", help='shortcut for --log-handler=modoo.http.rpc.request:DEBUG')
        group.add_option('--log-response', action="append_const", dest="log_handler", const="modoo.http.rpc.response:DEBUG", help='shortcut for --log-handler=modoo.http.rpc.response:DEBUG')
        group.add_option('--log-web', action="append_const", dest="log_handler", const="modoo.http:DEBUG", help='shortcut for --log-handler=modoo.http:DEBUG')
        group.add_option('--log-sql', action="append_const", dest="log_handler", const="modoo.sql_db:DEBUG", help='shortcut for --log-handler=modoo.sql_db:DEBUG')
    ...
    def parse_config(self, args=None):
        ...
        This method initializes modoo.tools.config and openerp.conf (the
        ...
        Typical usage of this method:
            ...
            modoo.tools.config.parse_config(sys.argv[1:])
        """
        opt = self._parse_config(args)
        modoo.netsvc.init_logger()
    ...
    def _parse_config(self, args=None):
        ...
        # TODO use appdirs
        if os.name == 'nt':
            rcfilepath = os.path.join(os.path.abspath(os.path.dirname(sys.argv[0])), 'modoo.conf')
        else:
            rcfilepath = os.path.expanduser('~/.modoorc')
            old_rcfilepath = os.path.expanduser('~/.openerp_serverrc')

            die(os.path.isfile(rcfilepath) and os.path.isfile(old_rcfilepath),
                "Found '.modoorc' and '.openerp_serverrc' in your path. Please keep only one of "\
                "them, preferably '.modoorc'.")

            if not os.path.isfile(rcfilepath) and os.path.isfile(old_rcfilepath):
                rcfilepath = old_rcfilepath

        self.rcfile = os.path.abspath(
            self.config_file or opt.config or os.environ.get('modoo_RC') or os.environ.get('OPENERP_SERVER') or rcfilepath)
        self.load()
    ...
    def _is_addons_path(self, path):
        from modoo.modules.module import MANIFEST_NAMES
