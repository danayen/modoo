...
from modoo.loglevels import get_encodings, ustr, exception_to_unicode     # noqa
...
def exec_pg_environ():
    ...
    """
    env = os.environ.copy()
    if modoo.tools.config['db_host']:
        env['PGHOST'] = modoo.tools.config['db_host']
    if modoo.tools.config['db_port']:
        env['PGPORT'] = str(modoo.tools.config['db_port'])
    if modoo.tools.config['db_user']:
        env['PGUSER'] = modoo.tools.config['db_user']
    if modoo.tools.config['db_password']:
        env['PGPASSWORD'] = modoo.tools.config['db_password']
    return env
...
def file_path(file_path, filter_ext=('',)):
    ...
    addons_paths = modoo.addons.__path__ + [root_path]
...
def file_open(name, mode="r", filter_ext=None):
    ...
    >>> with file_open('/opt/modoo/addons/hr/static/description/icon.png', 'rb') as f:
    ...     contents = f.read()
...
def scan_languages():
    ...
    :rtype: [(str, unicode)]
    """
    csvpath = modoo.modules.module.get_resource_path('base', 'data', 'res.lang.csv')
    try:
...
class mute_logger(object):
    ...
    Can be used as context manager or decorator.
        ...
        @mute_logger('modoo.plic.ploc')
        def do_stuff():
            blahblah()

        with mute_logger('modoo.foo.bar'):
            do_suff()
...
def format_date(env, value, lang_code=False, date_format=False):
    ...
    if isinstance(value, str):
        ...
        if len(value) > DATE_LENGTH:
            ...
            value = modoo.fields.Datetime.from_string(value)
            value = modoo.fields.Datetime.context_timestamp(env['res.lang'], value)
        else:
            value = modoo.fields.Datetime.from_string(value)
...
def format_datetime(env, value, tz=False, dt_format='medium', lang_code=False):
    ...
    if not value:
        return ''
    if isinstance(value, str):
        timestamp = modoo.fields.Datetime.from_string(value)
    else:
        timestamp = value
