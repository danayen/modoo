"""
This code is what let us use ES6-style modules in modoo.
Classic modoo modules are composed of a top-level :samp:`modoo.define({name},{body_function})` call.
This processor will take files starting with an `@modoo-module` annotation (in a comment) and convert them to classic modules.
If any file has the /** modoo-module */ on top of it, it will get processed by this class.
It performs several operations to get from ES6 syntax to the usual modoo one with minimal changes.
...
def transpile_javascript(url, content):
    """
    Transpile the code from native JS modules to custom modoo modules.
    ...
    legacy_modoo_define = get_aliased_modoo_define_content(module_path, content)
    ...
    steps = [
        ...
        partial(wrap_with_modoo_define, module_path),
    ]
    for s in steps:
        content = s(content)
    if legacy_modoo_define:
        content += legacy_modoo_define
    return content
...
def url_to_module_path(url):
    """
    modoo modules each have a name. (modoo.define("<the name>", async function (require) {...});
...
def wrap_with_modoo_define(module_path, content):
    """
    Wraps the current content (source code) with the modoo.define call.
    Should logically be called once all other operations have been performed.
    """
    return f"""modoo.define({module_path!r}, async function (require) {{
'use strict';
let __exports = {{}};
{content}
return __exports;
...
modoo_MODULE_RE = re.compile(r"""
    \s*                                       # some starting space
    \/(\*|\/).*\s*                            # // or /*
    @modoo-module                              # @modoo-module
...
def is_modoo_module(content):
    """
    Detect if the file is a native modoo module.
    We look for a comment containing @modoo-module.

    :param content: source code
    :return: is this a modoo module that need transpilation ?
    """
    result = modoo_MODULE_RE.match(content)
    return bool(result)


def get_aliased_modoo_define_content(module_path, content):
    """
    ...
    Example:
    If we have a require call somewhere in the modoo source base being:
    ...
    > modoo.define("web/chrome/abstract_action", async function(require) {
    ...
    .. code-block:: javascript
        ...
        /** @modoo-module */
        // after
        /** @modoo-module alias=web.AbstractAction */
    ...
    .. code-block:: javascript
        ...
        /** @modoo-module */
        // after
        /** @modoo-module alias=web.AbstractAction default=false */

    :return: the alias content to append to the source code.
    """
    matchobj = modoo_MODULE_RE.match(content)
    if matchobj:
        alias = matchobj['alias']
        if alias:
            if matchobj['default']:
                return """\nmodoo.define(`%s`, async function(require) {
                        return require('%s');
                        });\n""" % (alias, module_path)
            else:
                return """\nmodoo.define(`%s`, async function(require) {
