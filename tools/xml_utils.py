...
from modoo.exceptions import UserError


class modoo_resolver(etree.Resolver):
    """modoo specific file resolver that can be added to the XML Parser.
...
def _check_with_xsd(tree_or_str, stream, env=None):
    ...
    :param env (modoo.api.Environment): If it is given, it enables resolving the
    ...
    if env:
        parser.resolvers.add(modoo_resolver(env))
