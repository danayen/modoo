...
import modoo
import modoo.tools as tools
import modoo.release as release
from modoo.tools import pycompat
...
# ad_paths is a deprecated alias, please use modoo.addons.__path__
@tools.lazy
def ad_paths():
    warnings.warn(
        '"modoo.modules.module.ad_paths" is a deprecated proxy to '
        '"modoo.addons.__path__".', DeprecationWarning, stacklevel=2)
    return modoo.addons.__path__
...
class AddonsHook(object):
    ...
    def find_module(self, name, path=None):
        if name.startswith('openerp.addons.') and name.count('.') == 2:
            warnings.warn(
                '"openerp.addons" is a deprecated alias to "modoo.addons".',
    ...
    def load_module(self, name):
        ...
        modoo_name = re.sub(r'^openerp.addons.(\w+)$', r'modoo.addons.\g<1>', name)

        modoo_module = sys.modules.get(modoo_name)
        if not modoo_module:
            modoo_module = importlib.import_module(modoo_name)

        sys.modules[name] = modoo_module

        return modoo_module

class modooHook(object):
    """ Makes modoo package also available as openerp """
    ...
    def find_module(self, name, path=None):
        ...
        if re.match(r'^openerp\b', name):
            warnings.warn(
                'openerp is a deprecated alias to modoo.',
                DeprecationWarning, stacklevel=2)
            return self
    ...
    def load_module(self, name):
        ...
        canonical = re.sub(r'^openerp(.*)', r'modoo\g<1>', name)
...
class UpgradeHook(object):
    """Makes the legacy `migrations` package being `modoo.upgrade`"""

    def find_module(self, name, path=None):
        if re.match(r"^modoo.addons.base.maintenance.migrations\b", name):
    ...
    def load_module(self, name):
        ...
        canonical_upgrade = name.replace("modoo.addons.base.maintenance.migrations", "modoo.upgrade")
...
def initialize_sys_path():
    """
    Setup the addons path ``modoo.addons.__path__`` with various defaults
    and explicit directories.
    """
    # hook modoo.addons on data dir
    dd = os.path.normcase(tools.config.addons_data_dir)
    if os.access(dd, os.R_OK) and dd not in modoo.addons.__path__:
        modoo.addons.__path__.append(dd)

    # hook modoo.addons on addons paths
    for ad in tools.config['addons_path'].split(','):
        ad = os.path.normcase(os.path.abspath(tools.ustr(ad.strip())))
        if ad not in modoo.addons.__path__:
            modoo.addons.__path__.append(ad)

    # hook modoo.addons on base module path
    base_path = os.path.normcase(os.path.abspath(os.path.join(os.path.dirname(os.path.dirname(__file__)), 'addons')))
    if base_path not in modoo.addons.__path__ and os.path.isdir(base_path):
        modoo.addons.__path__.append(base_path)

    # hook modoo.upgrade on upgrade-path
    from modoo import upgrade
    ...
    # create decrecated module alias from modoo.addons.base.maintenance.migrations to modoo.upgrade
    spec = importlib.machinery.ModuleSpec("modoo.addons.base.maintenance", None, is_package=True)
    maintenance_pkg = importlib.util.module_from_spec(spec)
    maintenance_pkg.migrations = upgrade
    sys.modules["modoo.addons.base.maintenance"] = maintenance_pkg
    sys.modules["modoo.addons.base.maintenance.migrations"] = upgrade

    # hook deprecated module alias from openerp to modoo and "crm"-like to modoo.addons
    if not getattr(initialize_sys_path, 'called', False): # only initialize once
        sys.meta_path.insert(0, UpgradeHook())
        sys.meta_path.insert(0, modooHook())
...
def get_module_path(module, downloaded=False, display_warning=True):
    ...
    """
    for adp in modoo.addons.__path__:
...
def get_module_filetree(module, dir='.'):
    ...
    files = modoo.tools.osutil.listdir(path, True)
...
def get_resource_from_path(path):
    ...
    """
    resource = False
    for adpath in modoo.addons.__path__:
...
def load_openerp_module(module_name):
    ...
    try:
        __import__('modoo.addons.' + module_name)
        ...
        if info['post_load']:
            getattr(sys.modules['modoo.addons.' + module_name], info['post_load'])()
...
def get_modules():
    ...
    plist = []
    for ad in modoo.addons.__path__:
