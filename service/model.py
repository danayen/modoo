...
import modoo
from modoo.exceptions import UserError, ValidationError
from modoo.models import check_method_name
from modoo.tools.translate import translate, translate_sql_constraint
from modoo.tools.translate import _
...
def dispatch(method, params):
    ...
    registry = modoo.registry(db).check_signaling()
    fn = globals()[method]
...
def check(f):
    ...
    def wrapper(___dbname, *args, **kwargs):
        ...
        def tr(src, ttype):
            ...
            with closing(modoo.sql_db.db_connect(dbname).cursor()) as cr:
        ...
        while True:
            try:
                if modoo.registry(dbname)._init and not modoo.tools.config['test_enable']:
                    raise modoo.exceptions.Warning('Currently, this database is not fully loaded and can not be used.')
                ...
                registry = modoo.registry(dbname)
...
def execute_cr(cr, uid, obj, method, *args, **kw):
    ...
    cr.reset()
    recs = modoo.api.Environment(cr, uid, {}).get(obj)
    if recs is None:
        raise UserError(_("Object %s doesn't exist", obj))
    result = modoo.api.call_kw(recs, method, args, kw)
...
def execute(db, uid, obj, method, *args, **kw):
    ...
    with modoo.registry(db).cursor() as cr:
