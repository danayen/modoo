...
import modoo
from modoo import SUPERUSER_ID
from modoo.exceptions import AccessDenied
import modoo.release
import modoo.sql_db
import modoo.tools
from modoo.sql_db import db_connect
from modoo.release import version_info
...
def check_db_management_enabled(method):
    def if_db_mgt_enabled(method, self, *args, **kwargs):
        if not modoo.tools.config['list_db']:
...
def check_super(passwd):
    if passwd and modoo.tools.config.verify_admin_password(passwd):
        return True
    raise modoo.exceptions.AccessDenied()

# This should be moved to modoo.modules.db, along side initialize().
def _initialize_db(id, db_name, demo, lang, user_password, login='admin', country_code=None, phone=None):
    try:
        db = modoo.sql_db.db_connect(db_name)
        with closing(db.cursor()) as cr:
            # TODO this should be removed as it is done by Registry.new().
            modoo.modules.db.initialize(cr)
            modoo.tools.config['load_language'] = lang
            cr.commit()

        registry = modoo.modules.registry.Registry.new(db_name, demo, None, update_module=True)

        with closing(registry.cursor()) as cr:
            env = modoo.api.Environment(cr, SUPERUSER_ID, {})
            ...
            if login:
                values['login'] = login
                emails = modoo.tools.email_split(login)
                if emails:
...
def _create_empty_database(name):
    db = modoo.sql_db.db_connect('postgres')
    with closing(db.cursor()) as cr:
        chosen_template = modoo.tools.config['db_template']
    ...
    if modoo.tools.config['unaccent']:
        try:
            db = modoo.sql_db.db_connect(name)
            with closing(db.cursor()) as cr:
...
def exp_duplicate_database(db_original_name, db_name):
    ...
    modoo.sql_db.close_db(db_original_name)
    db = modoo.sql_db.db_connect('postgres')
    with closing(db.cursor()) as cr:
        # database-altering operations cannot be executed inside a transaction
        cr._cnx.autocommit = True
    ...
    registry = modoo.modules.registry.Registry.new(db_name)
    with registry.cursor() as cr:
        # if it's a copy of a database, force generation of a new dbuuid
        env = modoo.api.Environment(cr, SUPERUSER_ID, {})
        env['ir.config_parameter'].init(force=True)

    from_fs = modoo.tools.config.filestore(db_original_name)
    to_fs = modoo.tools.config.filestore(db_name)
...
def exp_drop(db_name):
    if db_name not in list_dbs(True):
        return False
    modoo.modules.registry.Registry.delete(db_name)
    modoo.sql_db.close_db(db_name)
    ...
    fs = modoo.tools.config.filestore(db_name)
...
def dump_db_manifest(cr):
    ...
    manifest = {
        'modoo_dump': '1',
        'db_name': cr.dbname,
        'version': modoo.release.version,
        'version_info': modoo.release.version_info,
        'major_version': modoo.release.major_version,
...
def dump_db(db_name, stream, backup_format='zip'):
    ...
    if backup_format == 'zip':
        with tempfile.TemporaryDirectory() as dump_dir:
            filestore = modoo.tools.config.filestore(db_name)
            ...
            modoo.tools.exec_pg_command(*cmd)
            if stream:
                modoo.tools.osutil.zip_dir(dump_dir, stream, include_dir=False, fnct_sort=lambda file_name: file_name != 'dump.sql')
            else:
                t=tempfile.TemporaryFile()
                modoo.tools.osutil.zip_dir(dump_dir, t, include_dir=False, fnct_sort=lambda file_name: file_name != 'dump.sql')
                t.seek(0)
                return t
    else:
        cmd.insert(-1, '--format=c')
        stdin, stdout = modoo.tools.exec_pg_command_pipe(*cmd)
        if stream:
...
def restore_db(db, dump_file, copy=False):
    ...
    with tempfile.TemporaryDirectory() as dump_dir:
        ...
        if modoo.tools.exec_pg_command(pg_cmd, *pg_args):
            raise Exception("Couldn't restore database")

        registry = modoo.modules.registry.Registry.new(db)
        with registry.cursor() as cr:
            env = modoo.api.Environment(cr, SUPERUSER_ID, {})
            if copy:
                # if it's a copy of a database, force generation of a new dbuuid
...
def exp_rename(old_name, new_name):
    modoo.modules.registry.Registry.delete(old_name)
    modoo.sql_db.close_db(old_name)
    ...
    old_fs = modoo.tools.config.filestore(old_name)
    new_fs = modoo.tools.config.filestore(new_name)
...
def exp_change_admin_password(new_password):
    modoo.tools.config.set_admin_password(new_password)
    modoo.tools.config.save()
    return True
...
def exp_migrate_databases(databases):
    for db in databases:
        ...
        modoo.tools.config['update']['base'] = True
        modoo.modules.registry.Registry.new(db, force_demo=False, update_module=True)
    return True
...
@modoo.tools.mute_logger('modoo.sql_db')
...
def list_dbs(force=False):
    if not modoo.tools.config['list_db'] and not force:
        raise modoo.exceptions.AccessDenied()

    if not modoo.tools.config['dbfilter'] and modoo.tools.config['db_name']:
        # In case --db-filter is not provided and --database is passed, modoo will not
        ...
        res = sorted(db.strip() for db in modoo.tools.config['db_name'].split(','))
        return res
    ...
    with closing(db.cursor()) as cr:
        try:
            ...
            res = [modoo.tools.ustr(name) for (name,) in cr.fetchall()]
        except Exception:
...
def list_db_incompatible(databases):
    """"Check a list of databases if they are compatible with this version of modoo
    ...
    for database_name in databases:
        with closing(db_connect(database_name).cursor()) as cr:
            if modoo.tools.table_exists(cr, 'ir_module_module'):
    ...
    for database_name in incompatible_databases:
        # release connection
        modoo.sql_db.close_db(database_name)
...
def exp_list_lang():
    return modoo.tools.scan_languages()

def exp_list_countries():
    list_countries = []
    root = ET.parse(os.path.join(modoo.tools.config['root_path'], 'addons/base/data/res_country_data.xml')).getroot()
