...
from .. import tools
from .common import TagsSelector, modooSuite
from .runner import modooTestResult
...
def get_test_modules(module):
    ...
    results = _get_tests_modules('modoo.addons', module)

    try:
        importlib.import_module('modoo.upgrade.%s' % module)
    except ImportError:
        pass
    else:
...
def _get_upgrade_test_modules(module):
    upg = importlib.import_module("modoo.upgrade")
    for path in map(Path, upg.__path__):
        for test in (path / module / "tests").glob("test_*.py"):
            spec = importlib.util.spec_from_file_location(f"modoo.upgrade.{module}.tests.{test.stem}", test)
            if not spec:
                continue
...
def make_suite(module_names, position='at_install'):
    ...
    return modooSuite(sorted(tests, key=lambda t: t.test_sequence))
