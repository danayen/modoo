# -*- coding: utf-8 -*-
"""
The module :mod:`modoo.tests.common` provides unittest test cases and a few
...
import modoo
from modoo import api
from modoo.models import BaseModel
from modoo.exceptions import AccessError
from modoo.modules.registry import Registry
from modoo.osv.expression import normalize_domain, TRUE_LEAF, FALSE_LEAF
from modoo.service import security
from modoo.sql_db import BaseCursor, Cursor
from modoo.tools import float_compare, single_email_re, profiler
from modoo.tools.misc import find_in_path
from modoo.tools.safe_eval import safe_eval
...
# The modoo library is supposed already configured.
ADDONS_PATH = modoo.tools.config['addons_path']
HOST = '127.0.0.1'
# Useless constant, tests are aware of the content of demo data
ADMIN_USER_ID = modoo.SUPERUSER_ID


def get_db_name():
    db = modoo.tools.config['db_name']
...
def standalone(*tags):
    ...
    ``tags`` and the corresponding modoo module name.
    """
    def register(func):
        # register func by modoo module name
        if func.__module__.startswith('modoo.addons.'):
...
def new_test_user(env, login='', groups='base.group_user', context=None, **kwargs):
     ...
     * email: it is either the login (if it is a valid email) or a generated
       ...
       to email being required for most modoo operations;
    """
    if not login:
...
# Main classes
# ------------------------------------------------------------
class modooSuite(unittest.suite.TestSuite):

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        from modoo.http import root
        if not root._loaded:
            root.load_addons()
            root._loaded = True
...
class MetaCase(type):
    ...
    def __init__(cls, name, bases, attrs):
        ...
        if cls.__module__.startswith('modoo.addons.'):
...
class BaseCase(unittest.TestCase, metaclass=MetaCase):
    """ Subclass of TestCase for modoo-specific code. This class is abstract and
    ...
    def browse_ref(self, xid):
        ...
        :param xid: fully-qualified :term:`external identifier`, in the form
                    :samp:`{module}.{identifier}`
        :raise: ValueError if not found
        :returns: :class:`~modoo.models.BaseModel`
        """
    ...
    def assertQueries(self, expected, flush=True):
        ...
        with patch('modoo.sql_db.Cursor.execute', execute):
            with patch('modoo.osv.expression.get_unaccent_wrapper', get_unaccent_wrapper):
        ...
        if self.warm:
            ...
            with patch('random.random', lambda: 1):
                ...
                if count != expected:
                    ...
                    if "/modoo/addons/" in filename:
                        filename = filename.rsplit("/modoo/addons/", 1)[1]
                    if count > expected:
    ...
    def _callSetUp(self):
        ...
        # See https://github.com/modoo/modoo/pull/107572 for more info.
...
class _ErrorCatcher(list):
    ...
    def _complete_traceback(self, initial_tb):
        ...
        # remove traceback root part (modoo_bin, main, loading, ...), as
...
class TransactionCase(BaseCase):
    ...
    def setUpClass(cls):
        super().setUpClass()
        cls.registry = modoo.registry(get_db_name())
        ...
        cls.env = api.Environment(cls.cr, modoo.SUPERUSER_ID, {})
...
class ChromeBrowser():
    ...
    def __init__(self, logger, window_size, test_class):
        ...
        self.request_id = 0
        self.user_data_dir = tempfile.mkdtemp(suffix='_chrome_modoo')
        self.chrome_pid = None

        otc = modoo.tools.config
    ...
    def _from_remoteobject(self, arg):
        ...
        # of the way modoo widgets are created they all appear as Class(...)
...
class HttpCase(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.xmlrpc_url = f'http://{HOST}:{modoo.tools.config["http_port"]:d}/xmlrpc/2/'
    ...
    def url_open(self, url, data=None, files=None, timeout=10, headers=None, allow_redirects=True, head=False):
        if url.startswith('/'):
            url = "http://%s:%s%s" % (HOST, modoo.tools.config['http_port'], url)
        if head:
    ...
    def _wait_remaining_requests(self, timeout=10):
        ...
        def get_http_request_threads():
            return [t for t in threading.enumerate() if t.name.startswith('modoo.service.http.request.')]
        ...
        if request_threads:
            ...
            modoo.tools.misc.dumpstacks()

    def logout(self, keep_db=True):
        self.session.logout(keep_db=True)
        modoo.http.root.session_store.save(self.session)

    def authenticate(self, user, password):
        db = get_db_name()
        if getattr(self, 'session', None):
            modoo.http.root.session_store.delete(self.session)

        self.session = session  = modoo.http.root.session_store.new()
        session.db = db
        ...
        modoo.http.root.session_store.save(session)
    ...
    @classmethod
    def base_url(cls):
        return "http://%s:%s" % (HOST, modoo.tools.config['http_port'])
    ...
    def start_tour(self, url_path, tour_name, step_delay=None, **kwargs):
        ...
        code = kwargs.pop('code', "modoo.startTour('%s'%s)" % (tour_name, step_delay))
        ready = kwargs.pop('ready', "modoo.__DEBUG__.services['web_tour.tour'].tours['%s'].ready" % tour_name)
    ...
    def profile(self, **kwargs):
        ...
        return profiler.Nested(_profiler, patch('modoo.http.Request._get_profiler_context_manager', route_profiler))
...
class Form(object):
    ...
    * properly handle defaults & onchanges around x2many fields
    ...
    :class:`~modoo.fields.Many2one` fields assign a singleton recordset::
    ...
    For :class:`~modoo.fields.Many2many` fields, the field itself is a
    :class:`~modoo.tests.common.M2MProxy` and can be altered by adding or
    removing records::
    ...
    Finally :class:`~modoo.fields.One2many` are reified as
    :class:`~modoo.tests.common.O2MProxy`.

    Because the :class:`~modoo.fields.One2many` only exists through its
    parent, it is manipulated more directly by creating "sub-forms"
    with the :meth:`~modoo.tests.common.O2MProxy.new` and
    :meth:`~modoo.tests.common.O2MProxy.edit` methods. These would
    ...
    :type recordp: modoo.models.Model
    ...
    :type view: int | str | modoo.model.Model
...
class O2MProxy(X2MProxy):
    ...
    def new(self):
        ...
        :class:`~modoo.fields.One2many` record, properly initialised.
    ...
    def edit(self, index):
        ...
        :class:`~modoo.fields.One2many` record.
...
def record_to_values(fields, record):
    ...
    for f, v in record.read(to_read)[0].items():
        ...
        if descr['type'] == 'many2one':
            ...
            v = modoo.fields.Datetime.to_string(v)
        elif descr['type'] == 'date' and isinstance(v, date):
            v = modoo.fields.Date.to_string(v)
        r[f] = v
    return r
...
def _cleanup_from_default(type_, value):
    ...
    if type_ == 'one2many':
        ...
        return modoo.fields.Datetime.to_string(value)
    elif type_ == 'date' and isinstance(value, date):
        return modoo.fields.Date.to_string(value)
    return value
...
def tagged(*tags):
    ...
    By default, all Test classes from modoo.tests.common have a test_tags
