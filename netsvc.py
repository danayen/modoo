...
class PostgreSQLHandler(logging.Handler):
    ...
    def emit(self, record):
        ...
        if not dbname:
            return
        with tools.ignore(Exception), tools.mute_logger('modoo.sql_db'), sql_db.db_connect(dbname, allow_uri=True).cursor() as cr:
...
def init_logger():
    ...
    warnings.filterwarnings('ignore', r'^Sampling from a set', category=DeprecationWarning, module='modoo')
    ...
    warnings.filterwarnings('ignore', category=BytesWarning, module='modoo.tools.image')
...
DEFAULT_LOG_CONFIGURATION = [
    'modoo.http.rpc.request:INFO',
    'modoo.http.rpc.response:INFO',
    ':INFO',
]
PSEUDOCONFIG_MAPPER = {
    'debug_rpc_answer': ['modoo:DEBUG', 'modoo.sql_db:INFO', 'modoo.http.rpc:DEBUG'],
    'debug_rpc': ['modoo:DEBUG', 'modoo.sql_db:INFO', 'modoo.http.rpc.request:DEBUG'],
    'debug': ['modoo:DEBUG', 'modoo.sql_db:INFO'],
    'debug_sql': ['modoo.sql_db:DEBUG'],
    'info': [],
    'runbot': ['modoo:RUNBOT', 'werkzeug:WARNING'],
    'warn': ['modoo:WARNING', 'werkzeug:WARNING'],
    'error': ['modoo:ERROR', 'werkzeug:ERROR'],
    'critical': ['modoo:CRITICAL', 'werkzeug:CRITICAL'],
}
