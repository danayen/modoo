...
# *broken*
NO_POSTMORTEM = (modoo.exceptions.AccessDenied,
                 modoo.exceptions.UserError,
                 modoo.exceptions.RedirectWarning)
    ...
    try:
        ...
        if rpc_request_flag or rpc_response_flag:
            ...
            if rpc_request and rpc_response_flag:
                modoo.netsvc.log(rpc_request, logging.DEBUG, '%s.%s' % (service_name, method), replace_request_password(params))

        threading.current_thread().uid = None
        threading.current_thread().dbname = None
        if service_name == 'common':
            dispatch = modoo.service.common.dispatch
        elif service_name == 'db':
            dispatch = modoo.service.db.dispatch
        elif service_name == 'object':
            dispatch = modoo.service.model.dispatch
        ...
        if rpc_request_flag or rpc_response_flag:
            ...
            if rpc_response_flag:
                modoo.netsvc.log(rpc_response, logging.DEBUG, logline, result)
            else:
                modoo.netsvc.log(rpc_request, logging.DEBUG, logline, replace_request_password(params), depth=1)

        return result
    except NO_POSTMORTEM:
        raise
    except Exception as e:
        _logger.exception(modoo.tools.exception_to_unicode(e))
        modoo.tools.debugger.post_mortem(modoo.tools.config, sys.exc_info())
        raise
...
class WebRequest(object):
    ...
    def __init__(self, httprequest):
        ...
        # dispatching phase in modoo.service.wsgi_server.application
        if self.db:
    ...
    @property
    def cr(self):
        """ :class:`~modoo.sql_db.Cursor` initialized for the current method call.
    ...
    @property
    def env(self):
        """ The :class:`~modoo.api.Environment` bound to current request. """
        if self._env is None:
            self._env = modoo.api.Environment(self.cr, self.uid, self.context)
        return self._env
    ...
    def _handle_exception(self, exception):
        ...
        if not isinstance(exception, NO_POSTMORTEM) \
            ...
            modoo.tools.debugger.post_mortem(
                modoo.tools.config, sys.exc_info())
...
def route(route=None, **kw):
    ...
    .. admonition:: CSRF Protection
        ...
        modoo implements token-based `CSRF protection
        ...
        * if the form is generated in Python, a csrf token is
          ...
          <modoo.http.WebRequest.csrf_token`, the
          :data:`~modoo.http.request` object is available by default
        ...
        * if the endpoint can be called by external parties (not from
          modoo) as e.g. it is a REST API or a `webhook
    ...
    def _handle_exception(self, exception):
        ...
        try:
            ...
            if not isinstance(exception, SessionExpiredException):
                ...
                elif isinstance(exception, (modoo.exceptions.UserError,
...
modoo URLs are CSRF-protected by default (when accessed with unsafe
HTTP methods). See
https://www.modoo.com/documentation/15.0/developer/reference/addons/http.html#csrf for
more details.

* if this endpoint is accessed through modoo via py-QWeb form, embed a CSRF
...
class ControllerType(type):
    def __init__(cls, name, bases, attrs):
        ...
        if not class_path[:2] == ["modoo", "addons"]:
            module = ""
        else:
...
class EndPoint(object):
    ...
    # When modoo clears the routing map, new EndPoint objects are created,
...
def _generate_routing_rules(modules, nodb_only, converters=None):
    def get_subclasses(klass):
        def valid(c):
            return c.__module__.startswith('modoo.addons.') and c.__module__.split(".")[2] in modules
...
class OpenERPSession(sessions.Session):
    ...
    def authenticate(self, db, login=None, password=None):
        ...
        uid = modoo.registry(db)['res.users'].authenticate(db, login, password, env)
        self.pre_uid = uid
    ...
    def check_security(self):
        ...
        env = modoo.api.Environment(request.cr, self.uid, self.context)
...
modoo_DISABLE_SESSION_GC = str2bool(os.environ.get('modoo_DISABLE_SESSION_GC', '0'))

if modoo_DISABLE_SESSION_GC:
...
class Response(werkzeug.wrappers.Response):
    ...
    def render(self):
        ...
        """
        env = request.env(user=self.uid or request.uid or modoo.SUPERUSER_ID)
...
class Root(object):
    ...
    def session_store(self):
        # Setup http sessions
        path = modoo.tools.config.session_dir
    ...
    def nodb_routing_map(self):
        ...
        for url, endpoint, routing in modoo.http._generate_routing_rules([''] + modoo.conf.server_wide_modules, True):
    ...
    def load_addons(self):
        ...
        for addons_path in modoo.addons.__path__:
    ...
    def dispatch(self, environ, start_response):
        ...
        try:
            ...
            with request_manager:
                ...
                if db:
                    try:
                        modoo.registry(db).check_signaling()
                        with modoo.tools.mute_logger('modoo.sql_db'):
...
def db_list(force=False, httprequest=None):
    try:
        dbs = modoo.service.db.list_dbs(force)
...
def db_filter(dbs, httprequest=None):
    ...
    if d == "www" and r:
        d = r.partition('.')[0]
    if modoo.tools.config['dbfilter']:
        d, h = re.escape(d), re.escape(h)
        r = modoo.tools.config['dbfilter'].replace('%h', h).replace('%d', d)
        dbs = [i for i in dbs if re.match(r, i)]
    elif modoo.tools.config['db_name']:
        # In case --db-filter is not provided and --database is passed, modoo will
        # use the value of --database as a comma separated list of exposed databases.
        exposed_dbs = set(db.strip() for db in modoo.tools.config['db_name'].split(','))
...
def send_file(filepath_or_fp, mimetype=None, as_attachment=False, filename=None, mtime=None,
    ...
    if isinstance(mtime, str):
        try:
            server_format = modoo.tools.misc.DEFAULT_SERVER_DATETIME_FORMAT
...
def content_disposition(filename):
    filename = modoo.tools.ustr(filename)
