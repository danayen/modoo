    ...
    *   :class:`AbstractModel` for abstract super classes meant to be shared by
    ...
    def load(self, fields, data):
        ...
        for field_path in fields:
            ...
            if isinstance(model_fields[field_path[0]], modoo.fields.Many2one):
            ...
            for field_name in field_path:
                ...
                if isinstance(model_fields[field_name], modoo.fields.One2many):
    ...
    def _add_fake_fields(self, fields):
        from modoo.fields import Char, Integer
    ...
    def user_has_groups(self, groups):
        ...
        :return: True if the current user is a member of one of the given groups
            ...
            preceded by ``!``
        """
        from modoo.http import request
        user = self.env.user
    ...
    def _read_group_fill_temporal(self, data, groupby, aggregated_fields, annotated_groupbys,
        ...
        if fill_from:
            fill_from = date_utils.start_of(modoo.fields.Datetime.to_datetime(fill_from), granularity)
            if tz:
                fill_from = tz.localize(fill_from)
        elif existing_from:
            fill_from = existing_from
        if fill_to:
            fill_to = date_utils.start_of(modoo.fields.Datetime.to_datetime(fill_to), granularity)
            if tz:
...
%(fields_list)s""") % {
    ...
    def _check_company(self, fnames=None):
        ...
        :raises UserError: if the `company_id` of the value of any field is not
            ...
            :class:`~modoo.addons.base.models.res_company`).

        For :class:`~modoo.addons.base.models.res_users` relational fields,
    ...
    def unlink(self):
        ...
        from modoo.addons.base.models.ir_model import MODULE_UNINSTALL_FLAG
    ...
    def write(self, vals):
        ...
        * For numeric fields (:class:`~modoo.fields.Integer`,
          :class:`~modoo.fields.Float`) the value should be of the
          corresponding type
        * For :class:`~modoo.fields.Boolean`, the value should be a
          :class:`python:bool`
        * For :class:`~modoo.fields.Selection`, the value should match the
          selection values (generally :class:`python:str`, sometimes
          :class:`python:int`)
        * For :class:`~modoo.fields.Many2one`, the value should be the
        ...
        * Other non-relational fields use a string for value
          ...
          .. danger::
              ...
              :class:`~modoo.fields.Date` and
              :class:`~modoo.fields.Datetime` fields use strings as values
              (written and read) rather than :class:`~python:datetime.date` or
              :class:`~python:datetime.datetime`. These date strings are
              UTC-only and formatted according to
              :const:`modoo.tools.misc.DEFAULT_SERVER_DATE_FORMAT` and
              :const:`modoo.tools.misc.DEFAULT_SERVER_DATETIME_FORMAT`
        * .. _openerp/models/relationals/format:

          The expected value of a :class:`~modoo.fields.One2many` or
          :class:`~modoo.fields.Many2many` relational field is a list of
          :class:`~modoo.fields.Command` that manipulate the relation the
          implement. There are a total of 7 commands:
          :meth:`~modoo.fields.Command.create`,
          :meth:`~modoo.fields.Command.update`,
          :meth:`~modoo.fields.Command.delete`,
          :meth:`~modoo.fields.Command.unlink`,
          :meth:`~modoo.fields.Command.link`,
          :meth:`~modoo.fields.Command.clear`, and
          :meth:`~modoo.fields.Command.set`.
          """
        if not self:
            return True
    ...
    def ensure_one(self):
        ...
        :raise modoo.exceptions.ValueError: ``len(self) != 1``
        """
        try:
    ...
    def with_env(self, env):
        ...
        :param env:
        :type env: :class:`~modoo.api.Environment`
    ...
    def with_company(self, company):
        ...
        :type company: :class:`~modoo.addons.base.models.res_company` or int
        ...
        for field, value in sorted(field_values, key=lambda item: item[0].write_sequence):
            ...
            if field.relational:
                ...
                for invf in self.pool.field_inverses[field]:
                    ...
                    # /home/dle/src/modoo/master-nochange-fp/modoo/addons/test_new_api/tests/test_new_fields.py
    ...
    def _populate_factories(self):
        ...
        See :mod:`modoo.tools.populate` for population tools and applications.
...
class Model(AbstractModel):
    """ Main super-class for regular database-persisted modoo models.

    modoo models are created by inheriting from this class::
